...
const UpdateProposal=({update, setUpdate})=>{
    ...
    const getProposalData = () => {
        axios.get(`https://event-proposal-page-94qh.onrender.com/events/${update}`, { withCredentials:true }).then(data=>{
    ...
    const uploadImages = async(val) => {
        ...
        data.append("upload_preset","venky");
    ...
    const clickHandler=async (e)=>{
        e.preventDefault();
        axios.put(`https://event-proposal-page-94qh.onrender.com/events/${update}`, allData).then(data=>{
            Swal.fire({
                position: 'center',
                icon: 'success',
